// https://github.com/udivankin/sunrise-sunset/blob/master/src/index.ts
// so we dont have to install something
// more reading on this math here: https://gml.noaa.gov/grad/solcalc/solareqns.PDF
/**
...
/**
 * Default latitude (Virginia for ~center of US)
 */
const DEFAULT_LAT = 37.4316;
...
const DEFAULT_LONG = (() => {
  ...
  const mod = timezoneOffset < 0 ? -1 : 1;

  return (timezoneOffset * 15) + (mod * 7.5);
})(); 
 ...
 */
export function getSunrise(latitude = DEFAULT_LAT, longitude = DEFAULT_LONG, date = new Date()): Date {
 ...
 */
export function getSunset(latitude = DEFAULT_LAT, longitude = DEFAULT_LONG, date = new Date()): Date {
